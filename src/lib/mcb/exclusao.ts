...
/** A confirmação é o e-mail da afiliada digitado de novo — ignora maiúsculas e espaços. */
 ...
 * pessoa, não da inscrição: se ela também é gestora, administra a plataforma ou se
...
export function motivoParaManterConta(situacao: SituacaoConta): string | null {
  ...
  if (situacao.outrasCandidaturas > 0) {
    return situacao.outrasCandidaturas === 1
      ? "a conta tem outra inscrição, em outro ambiente."
      : `a conta tem outras ${situacao.outrasCandidaturas} inscrições, em outros ambientes.`;
  }
  return null;
}
 ...
 * afiliada é `{tenant_id}/{influencer_id}/`; entradas vazias ou de subpasta ficam fora.
 */
