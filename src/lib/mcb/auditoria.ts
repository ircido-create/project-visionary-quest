 ...
 * afiliada seguir para a análise oficial. Até aqui ela só tinha um contador no painel,
 * e as afiliadas paravam ali. As regras abaixo decidem o que cada decisão faz; quem
...
/** Para onde a afiliada pode voltar. Nunca para frente, nem para fora da jornada. */
...
export function validarDecisao(entrada: {
  ...
  if (entrada.etapaAtual !== ETAPA_EM_AUDITORIA) {
    return {
      ok: false,
      erro: `A auditoria só vale para afiliadas em "${STATUS_LABELS[ETAPA_EM_AUDITORIA]}". Esta está em "${STATUS_LABELS[entrada.etapaAtual]}".`,
    };
  }
  ...
  if (nota.length < NOTA_MINIMA) {
    return {
      ok: false,
      erro: `Devolver exige o motivo, com pelo menos ${NOTA_MINIMA} caracteres: ele vira a tarefa da afiliada.`,
    };
  }
 ...
 * 2026-09-12: sem isto, o seletor comum de etapa levava uma afiliada direto para
...
export function motivoBloqueioSeletor(de: InfluencerStatus, para: InfluencerStatus): string | null {
  ...
  if (para === ETAPA_APROVADA && !jaAuditada) {
    return `Para chegar a "${STATUS_LABELS[ETAPA_APROVADA]}", use a auditoria na página da afiliada: é ela que registra quem aprovou e com base em quê.`;
  }
  if (ETAPAS_DEPOIS_DA_AUDITORIA.includes(para) && !jaAuditada) {
    return `"${STATUS_LABELS[para]}" só vale para afiliadas que já passaram pela auditoria.`;
  }
  return null;
}
