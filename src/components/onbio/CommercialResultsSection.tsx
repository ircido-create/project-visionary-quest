...
export function CommercialResultsSection({
  tenantId,
  influencerId,
  readOnly,
}: {
  ...
  const results = useQuery({
    queryKey: ["onbio", "results", tenantId, influencerId],
    queryFn: () => fetchResults({ data: { tenantId, influencerId } }),
  });
  const invalidate = () => {
    client.invalidateQueries({ queryKey: ["onbio", "results", tenantId, influencerId] });
  };
    ...
    mutationFn: () =>
      saveResult({
        data: {
          ...
          revenueCents: Math.round(Number(form.revenue || 0) * 100),
          orders: Number(form.orders || 0),
          commissionCents: Math.round(Number(form.commission || 0) * 100),
          goalCents: form.goal ? Math.round(Number(form.goal) * 100) : null,
  ...
  const removeMutation = useMutation({
    mutationFn: (id: string) => removeResult({ data: { tenantId, id } }),
    onSuccess: () => {
      invalidate();
      ...
      <section className="glass rounded-xl border border-border/60 p-6 lg:col-span-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="flex items-center gap-2 text-xl font-semibold">
              <BarChart3 className="h-5 w-5" /> Resultados comerciais
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Faturamento, pedidos, comissão e metas por período.
            </p>
          </div>
          <div className="text-right">
            <p className="text-xs text-muted-foreground">Faturamento acumulado</p>
            <p className="text-2xl font-semibold text-primary">{money(total)}</p>
          </div>
        </div>
        {!readOnly ? (
          <form
          ...
          >
            ...
            <Field label="Campanha ou produto">
              <Input
                value={form.campaign}
            ...
            <Button className="md:col-span-4 md:w-fit" disabled={mutation.isPending}>
              {mutation.isPending ? "Salvando..." : "Registrar resultado"}
            </Button>
          </form>
        ) : null}
        <div className="mt-6 overflow-x-auto">
            ...
            <tbody>
              {(results.data ?? []).map((row) => (
                <tr key={row.id} className="border-t border-border/60">
                  <td className="py-3">
                    {new Date(`${row.period_start}T12:00:00`).toLocaleDateString("pt-BR")} –{" "}
          ...
          {!results.isLoading && (results.data ?? []).length === 0 ? (
            <p className="py-6 text-sm text-muted-foreground">Nenhum resultado registrado.</p>
          ) : null}
        </div>
      </section>
    </>
